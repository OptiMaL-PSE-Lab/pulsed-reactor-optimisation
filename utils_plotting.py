...
def plot_data_file(path):
    ...
    ax[0].scatter(np.arange(n),pred_s_obj_mean_unnorm,c='tab:red',s=ms,label='Predicted')
    ax[0].scatter(np.arange(n),obj,c='k',s=ms,label='Evaluated')
    for i in range(n):
        ax[0].plot([i,i],[pred_s_obj_mean_unnorm[i],obj[i]],c='k',lw=3,ls='dotted')

    ax[1].scatter(np.arange(n),pred_s_cost_mean_unnorm,c='tab:red',s=ms,label='Predicted')
    ax[1].scatter(np.arange(n),cost,c='k',s=ms,label='Evaluated')
    for i in range(n):
        ax[1].plot([i,i],[pred_s_cost_mean_unnorm[i],cost[i]],c='k',lw=3,ls='dotted')
    ...
    ax[0].plot(np.arange(n),max_s+max_g,c='tab:blue',lw=3,label='Max Predicted: Standard + Greedy')
    ...
    ax[1].plot(time,max_s+max_g,c='tab:blue',lw=3,label='Max Predicted: Standard + Greedy')
...
def plot_results(path):
    ...
    im1 = ax[0,0].scatter(full_it, obj, c=cost, marker=mar, s=ms, lw=0,edgecolor='w',norm=norm,alpha=m_alpha)
    ...
    for i in range(len(full_it)-1):
        ax[0,0].fill_between([full_it[i],full_it[i+1]],[obj[i],obj[i+1]],[0,0],alpha=0.5,color=rgba_color[i+1],zorder=-1)
        ax[0,0].fill_between([full_it[i],full_it[i+1]],[best_obj[i],best_obj[i+1]],[obj[i],obj[i+1]],color='k',alpha=0.1,lw=0,zorder=-1)
    ...
    for i in range(len(it)-1):
        ax[0,1].fill_between([it[i],it[i+1]],[best_crit[i],best_crit[i+1]],[0,0],color='k',alpha=0.1,lw=0,zorder=-1)


    im2 = ax[1,0].scatter(full_time, obj, c=cost, marker=mar, s=ms, lw=0,edgecolor='w',norm=norm,alpha=m_alpha)
    ...
    for i in range(len(full_it)-1):
        ax[1,0].fill_between([full_time[i],full_time[i+1]],[obj[i],obj[i+1]],[0,0],alpha=0.5,color=rgba_color[i+1],zorder=-1)
        ax[1,0].fill_between([full_time[i],full_time[i+1]],[best_obj[i],best_obj[i+1]],[obj[i],obj[i+1]],color='k',alpha=0.1,lw=0,zorder=-1)
    ...
    for i in range(len(it)-1):
        ax[1,1].fill_between([time[i],time[i+1]],[best_crit[i],best_crit[i+1]],[0,0],color='k',alpha=0.1,lw=0,zorder=-1)
