...
def rotate_x_simple(x0, y0, z0, r_x):
        # rotation of points around x axis by r_x radians
        y = np.array(y0) - y0[0]
        z = np.array(z0) - z0[0]
        y_new = y * np.cos(r_x) - z * np.sin(r_x) + y0[0]
        z_new = y * np.sin(r_x) - z * np.cos(r_x) + z0[0]
        return x0, y_new, z_new

def rotate_y_simple(x0, y0, z0, r_y):
        # rotation of points around x axis by r_x radians
        x = np.array(x0) - x0[0]
        z = np.array(z0) - z0[0]
        z_new = z * np.cos(r_y) - x * np.sin(r_y) + z0[0]
        x_new = z * np.sin(r_y) - x * np.cos(r_y) + x0[0]
        return x_new, y0, z_new


def rotate_z_simple(x0, y0, z0, r_z):
        # rotation of points around x axis by r_x radians
        ...
        y = np.array(y0) - y0[0]
        # ax.plot(x,y,z0)
        x_new = x * np.cos(r_z) - y * np.sin(r_z) + x0[0]
        y_new = x * np.sin(r_z) + y * np.cos(r_z) + y0[0]
        # ax.plot(x_new,y_new,z0)
        # plt.show()
        return x_new, y_new, z0
...
def interpolate_path(r,theta,z,f):
        ...
        rho,theta,z = cartesian_convert(x_m,y_m,z_m)
        return rho,theta,z
...
def interpolate_split(r,theta,z,fid_ax):
        #z[1] = (z[0]+z[2])/2
        x,y,z = cylindrical_convert(r,theta,z)
        rho_mid = interpolate_s(r, fid_ax, "quadratic")
        theta_mid = interpolate_s(theta, fid_ax, "quadratic")
        z_mid = interpolate_s(z, fid_ax, "quadratic")
        x_m,y_m,z_m = cylindrical_convert(rho_mid,theta_mid,z_mid)
...
def create_mesh(data,interp_points, path, n_interp, nominal_data_og,repeats):
        ...
        data['rho'],data['theta'],data['z'] = cartesian_convert(x,y,z)
        ...
        fig,ax = plt.subplots(1,1,subplot_kw={'projection':'3d'})
        ax.plot(data['x'],data['y'],data['z'],c='k')
        fig.savefig(path+'/path.png')
        ...
        for i in range(len(p_cylindrical_list[:,0,0])):
                ...
                x,y,z = interpolate_split(p_cylindrical_list[i,0,:],p_cylindrical_list[i,1,:],p_cylindrical_list[i,2,:], fid_ax)
        ...
        fig_p.tight_layout()
        fig_p.savefig(path + "/path_reactor.png", dpi=600)
        p_list = p_list.T
...
for i in range(2,n):
        ...
        if i %2 == 0:
                data['rho_'+str(i)] = -0.003
        else:   
                data['rho_'+str(i)] = 0.0025
...
tr = 0.00125
n_r = 6
x_add = [tr for i in range(n_r)]
x_con = [0.002,0.002,0.0005,0.002,0.002,0.0005]
x_list.append(np.array(x_add))
for i in range(n_circ):
        x_list.append(np.array(x_add))
        # x_list.append(np.array(x_add))
        x_list.append(np.array(x_con))
x_list.append(np.array(x_add))
x_list.append(np.array(x_add))

repeats = 1
create_mesh(data,x_list,'mesh_generation/experimental_both',n,nominal_data,repeats)
...
repeats = 1
create_mesh(data,x_list,'mesh_generation/experimental_cross',n,nominal_data,repeats)
...
repeats = 1
create_mesh(data,x_list,'mesh_generation/experimental_control',n,nominal_data,repeats)