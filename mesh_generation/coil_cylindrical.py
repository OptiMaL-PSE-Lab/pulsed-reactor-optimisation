...
def create_mesh(data, path):
	...
	interpolation_factor = 10  # interpolate 10 times the points between
	...
	for p in range(2,le-3):
		# get proceeding circle (as x,y,z samples)
		x2, y2, z2 = create_circle([data[keys[i]][p-1] for i in range(len(keys))],[data[keys[i]][p] for i in range(len(keys))],tube_rad)
		# get next circle (as x,y,z samples)
		x1, y1, z1 = create_circle([data[keys[i]][p] for i in range(len(keys))],[data[keys[i]][p+1] for i in range(len(keys))],tube_rad)
		# plot for reference 
		ax.plot3D(x1,y1,z1,c='k',alpha=0.5,lw=0.5)
...
n = coils*8 # points per coil to use 

# example data for cylindrical coordinates (normally optimized for)
# as close as possible to the paper 
data = {}
data["rho"] = [(-(x-coils*np.pi)**2+(coils*np.pi)**2)/(((coils*np.pi)**2)/(coil_rad_max)) for x in np.linspace(0,2*coils*np.pi,n)]
