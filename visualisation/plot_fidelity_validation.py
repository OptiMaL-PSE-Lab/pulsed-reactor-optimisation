...
from mf_experimental_design.utils import calculate_N,loss, calc_etheta
...
for e in [1,2]:
	...
	axs[i].scatter(theta,etheta,c='k',marker='o',alpha=0.8,label='Exp',lw=0,zorder=-1)
	axs[i].plot(t_theta,t_etheta,c='k',label='Modelled',lw=2,ls='--')
	axs[i].legend(frameon=False)
fig.tight_layout()
plt.savefig('outputs/n_grid.pdf',dpi=600)
...
# 	axs[i].scatter(theta,etheta,c='k',marker='o',alpha=0.6,s=30,label='Exp',lw=0,zorder=-1)
# 	axs[i].legend(frameon=False)
# fig.tight_layout()
# plt.savefig('outputs/fidelity_validation_profile.pdf',dpi=600)
...
# fig.tight_layout()
# plt.savefig('outputs/fidelity_validation_n.pdf',dpi=600)
